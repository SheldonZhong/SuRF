...
namespace surf
{
                                     ...
                                     position_t &out_node_num_right) const
    {
        ...
        for (level_t i = 0; i < height_; i++)
        {
            ...
            if (left_pos < right_pos)
            {
                ...
                position_t num_leafs = (rank_right_label - rank_left_label) - (rank_right_ind - rank_left_ind) + (rank_right_prefix - rank_left_prefix);
    ...
    uint64_t LoudsDense::serializedSize() const
    {
        ...
        sizeAlign(size);
        size += (label_bitmaps_->serializedSize() + child_indicator_bitmaps_->serializedSize() + prefixkey_indicator_bits_->serializedSize() + suffixes_->serializedSize());
        sizeAlign(size);
        return size;
    }
    ...
    {
        return (sizeof(LoudsDense) + label_bitmaps_->size() + child_indicator_bitmaps_->size() + prefixkey_indicator_bits_->size() + suffixes_->size());
    }
    ...
    position_t LoudsDense::getSuffixPos(const position_t pos, const bool is_prefix_key) const
    {
        ...
        position_t suffix_pos = (label_bitmaps_->rank(pos) - child_indicator_bitmaps_->rank(pos) + prefixkey_indicator_bits_->rank(node_num) - 1);
